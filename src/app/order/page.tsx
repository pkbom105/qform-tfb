...
export default function OrderForm() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-xl my-10 border border-slate-100">
      <h1 className="text-2xl font-light text-slate-800 mb-6 border-b pb-4">แบบฟอร์มขอใบเสนอราคา - Toffy Boutique</h1>
      
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* ส่วนข้อมูลผู้ติดต่อ */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-black mb-1">ชื่อผู้ติดต่อ *</label>
          ...
          <div>
            <label className="block text-sm font-medium text-black mb-1">บริษัท / นามบุคคล</label>
          ...
          <div>
            <label className="block text-sm font-medium text-black mb-1">เบอร์โทรศัพท์ *</label>
        ...
        <div>
          <label className="block text-sm font-light text-slate-800 mb-3">ชนิดสินค้าที่ต้องการสั่งผลิต *</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-black mb-1">เนื้อผ้าที่ต้องการ</label>
            <input name="fabricType" className="w-full p-2 border rounded-md" placeholder="เช่น Cotton 100%, TK, เนื้อเรียบ" />
          </div>
          <div>
            <label className="block text-sm font-medium text-black mb-1">จำนวนที่ต้องการ</label>
            <input name="quantity" className="w-full p-2 border rounded-md" placeholder="ระบุจำนวน (ตัว)" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-black mb-1">รายละเอียดรูปแบบเพิ่มเติม</label>
        ...
        <div className="flex flex-col items-center border-t pt-6">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full md:w-64 py-3 px-6 text-white font-light rounded-full transition ${loading ? 'bg-slate-400 cursor-not-allowed' : 'bg-slate-900 hover:bg-blue-700 shadow-lg'}`}
          >
