    ...
    >
      ...
      <div className="grid grid-cols-2 gap-y-3 gap-x-12 text-sm mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-100">
        <p>
          ...
          <span className="text-slate-600">{customer_profile.name}</span>
        </p>
        <p>
        ...
        <p>
          ...
          <span className="text-slate-600">{customer_profile.company}</span>
        </p>
        <p>
          <strong>อีเมล:</strong>{" "}
          <span className="text-slate-600">{customer_profile.email}</span>
        </p>
        <p>
          <strong>Line ID:</strong>{" "}
          <span className="text-slate-600">{customer_profile.line_id}</span>
        </p>
        <p>
        ...
        <table className="w-full text-center border-collapse">
          ...
          <tbody>
            <tr className="text-[10px] bg-white">
              <td className="p-2 border-r border-slate-100 font-bold bg-slate-50">
                ขนาดรอบอก (นิ้ว)
              </td>
              {sizeList.map((s) => (
                <td
                  key={s}
                  className="p-2 border-r border-slate-100 text-red-600 font-bold"
                >
            ...
            <tr className="text-[11px] bg-slate-50">
              ...
              {sizeList.map((s) => (
                <td
                  key={s}
                  className="p-2 border-r border-slate-200 font-black"
                >
                  {product_specification.size_breakdown[s]?.qty || "0"}
                </td>
              ))}
              <td className="p-2 bg-red-50 text-red-600 font-black">
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="space-y-3">
          <div className="p-4 border-l-4 border-red-600 bg-slate-50 rounded-r-xl">
            ...
            <p className="text-sm font-bold">
              {decoration_details.printing_title} (
              {decoration_details.printing_size})
            </p>
          </div>
          <div className="p-4 border-l-4 border-slate-900 bg-slate-50 rounded-r-xl">
            ...
            <p className="text-sm font-bold">
              {decoration_details.embroidery_title} (
              {decoration_details.embroidery_size})
            </p>
          </div>
        </div>
        ...
        <div className="p-5 bg-yellow-50 border border-yellow-100 rounded-2xl relative overflow-hidden">
          ...
          <h4 className="text-[10px] font-black text-yellow-700 uppercase mb-2">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-4">
            {design_images.map((img: string, idx: number) => (
              <div
                key={idx}
                className="w-32 h-32 border-2 border-slate-100 rounded-xl overflow-hidden bg-slate-50 shadow-sm"
              >
                <img
                  src={img}
                  alt={`Attached ${idx}`}
